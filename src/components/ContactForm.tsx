...
const ContactForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col gap-2'>
        ...
        <input onChange={handleInput} required type="text" value={contactMessage.firstName} name="firstName" id="firstName" className='border border-gray-300 md:w-3/4 rounded-sm p-2'/>
        </div>
        <div>
        <label className='font-bold text-sm capitalize mb-2' htmlFor="lastName">Last Name</label> <br />
        <input onChange={handleInput} required type="text" value={contactMessage.lastName} name="lastName" id="lastName" className='border border-gray-300 md:w-3/4 rounded-sm p-2'/>
        </div>
        <div>
        <label className='font-bold text-sm capitalize mb-2' htmlFor="email">Email</label> <br />
        <input onChange={handleInput} required type="email" value={contactMessage.email} name="email" id="email" className='border border-gray-300 md:w-3/4 rounded-sm p-2'/>
        </div>
        <div>
        <label className='font-bold text-sm capitalize mb-2' htmlFor="message">Message</label> <br />
        <textarea onChange={handleTextArea} required name="message" value={contactMessage.message} id="message" rows={5} className='border border-gray-300 md:w-3/4 rounded-sm p-2'></textarea>
        </div>
